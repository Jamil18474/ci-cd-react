...
test('should register a new user and display it in the list', () => {
  ...
  const fillOutForm = () => {
    ...
    fireEvent.change(email, { target: { value: 'julien.dubout@example.com' } });
  ...
  expect(screen.getByText(/Date de naissance: 1990-01-01, Ville: Paris, Code postal: 75001/i)).toBeInTheDocument();
});
